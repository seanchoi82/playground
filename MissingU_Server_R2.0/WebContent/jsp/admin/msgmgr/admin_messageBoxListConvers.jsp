								...
								<c:choose>
									<c:when test="${ item.senderId eq reqParams.senderId }">
										<li class='sender'>
											...
											<span class='msg'>${ item.msgText } </span>
											<span class='date'>${ item.sendDate }<var data-msg-id='${ item.msgId }'>${ item.receiverReadYn }</var><em data-msg-id='${ item.msgId }'>[삭제]</em></span>
										</li>
									</c:when>
									<c:otherwise>
									<li class='receiver'>
...
$(document).on("click", ".msgbox li span em", function() {
	...
	$.ajax({
		...
		type : "POST",
		cache : false,
		data : { msgIds:$(this).attr("data-msg-id")}, 
		dataType : "json",
		success : function(data) {
