	...
	<div class="body">
		...
		<div class="content">
			<div class="ui-box ui-box-white ui-box-shadow">
				...
				<div class="pt30">
					<table class="tbl_nor" cellspacing="0" summary="쪽지 목록">
						...
						<tr id="tb_header">
							<th><input type="checkbox" id="replyAll" data-name="members" class="checkInverse"/></th>
							<th>발신자</th>
							<th></th>
							<th>수신자</th>
							<th>최근메세지</th>
						...
						<c:forEach items="${response.dataList}" var="item">
							<tr class="record">
								<td align="center"><input type="checkbox" name="members" value="${ item.senderId }:${ item.receiverId }" /></td>
								<td>
								...
								<td align="center">
									<a href="msgConversConversation.html?senderId=${ item.senderId }&receiverId=${ item.receiverId }" target="_blank">${item.lastMsgText}(${item.unreadMsgCnt})</a>
								</td>
								<td align="center">${item.lastSendDate}</td>
							</tr>
						</c:forEach>
					</table>
				</div>
			...
			<jsp:include page="/jsp/common/paging.jsp">
			     <jsp:param name="actionPath" value="msgConversList.html"/>
...
function deleteMsg() {
	...
	$.ajax({
		url : "deleteMsgConversAllArray.html",
		type : "POST",
		cache : false,
		data : { members:ids}, 
		dataType : "json",
		success : function(data) {
			...
			if(rsltCd == 0) { //저장성공
				alert("삭제 되었습니다.");
				location.href="msgConversList.html";
			} else {
				alert(rsltMsg);
			}
		},
